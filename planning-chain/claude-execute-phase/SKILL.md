...
PREDECESSOR_HANDOFF="$HOME/.claude/skills/claude-plan-phase/handoffs/${REPO_KEY}/latest.md"
```
...
if [[ -f ~/.claude/skills/_shared/next_reflection_path.py ]]; then
  REFLECTION_PATH=$(python3 ~/.claude/skills/_shared/next_reflection_path.py claude-execute-phase)
else
  REFLECTION_PATH=~/.claude/skills/claude-execute-phase/reflections/$(date -u +%Y-%m-%dT%H-%M-%SZ).md
fi
REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
if [ -z "$REPO_ROOT" ]; then
  REPO_KEY="_no-git-$(pwd | sha1sum | cut -c1-12)"
...
HANDOFF_DIR="$HOME/.claude/skills/claude-execute-phase/handoffs/${REPO_KEY}"
mkdir -p "$HANDOFF_DIR"
HANDOFF_PATH="$HANDOFF_DIR/latest.md"
SKILL_MD=~/.claude/skills/claude-execute-phase/SKILL.md
```
